...
try {
  ...
  packageDefinition = protoLoader.loadSync(path.resolve(PROTO_PATH, 'metaservicets.proto'), options);
}
...
export default {
  ...
  listennorthsouth(grpcObj: any) {
    ...
    log(`GRPC NORTH SOUTH serving on ${GRPC_HOST}:${GRPC_NORTH_SOUTH_PORT}`)
  },
  listeneastwest(grpcObj: any) {
    const service: any = packageObject['services'];
